...
public class Flink11_Practice_ItemCountTopN {
    ...
    public static class ItemCountWindowFunc implements WindowFunction<Integer, ItemCount, Long, TimeWindow> {
        ...
        public void apply(Long itemId, TimeWindow window, Iterable<Integer> input, Collector<ItemCount> out) throws Exception {
            ...
            // 输出数据
            out.collect(new ItemCount(itemId, new Timestamp(window.getEnd()).toString(),
                    count));
        }
    }
    ...
    public static class ItemCountProcessFunc extends KeyedProcessFunction<String, ItemCount, String> {
        ...
        public void onTimer(long timestamp, KeyedProcessFunction<String, ItemCount, String>.OnTimerContext ctx, Collector<String> out) throws Exception {
            // 取出状态中的所有数据
            Iterable<ItemCount> iterator = listState.get();

            // 将迭代器转换为集合
            ArrayList<ItemCount> itemCounts = Lists.newArrayList(iterator.iterator());
            ...
            for (int i = 0; i < Math.min(topSize, itemCounts.size()); i++) {
                ...
                builder.append(" Count：").append(itemCount.getCount());
                builder.append("\n");
            }
